# / Household selection from OB data source
...
household_selection <- function(data, 
  ...
  write_csv(sample, list_save)
  
  ggplot() + 
    geom_sf(data = shapefile, fill = NA) +
    geom_sf(data = sample, color = "black", shape = 17, size = 3) + 
    theme_bw() + 
...
replacement_selection <- function(data, 
  ...
  replacement_list <- as.data.frame(replacement_list)
    ...
    write_csv(replacement_list, list_save)
